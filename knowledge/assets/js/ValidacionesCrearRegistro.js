...
function validarGuardar(){
  ...
  var descpricion = document.getElementById("txtDescripcion").value;
  if (titulo==="" || descpricion===""){
    alert("Error al guardar, campos vacíos.");
    return false;
  }else{

    if(/^[0-9]+$/.test(titulo) || /^[0-9]+$/.test(descpricion)){
      alert("Error al guardar, no se permiten solo números en los campos.")
      return false;
    }else{
